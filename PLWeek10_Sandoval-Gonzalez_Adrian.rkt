...
(define (draw-to-screen poly)
  ...
  (send mySquare scale screenScale screenScale)

  ; Translate polygon for screen
  (send mySquare translate xTrans yTrans)
  ...
  (send mySquare translate (- 0 xTrans) (- 0 yTrans))

  ; Scale polygon for world
  (send mySquare scale (/ 1 screenScale) (/ 1 screenScale))  
  )
...
; Draw og square
(draw-to-screen mySquare)

; Translate and draw again
(send mySquare translate 50 50)
(draw-to-screen mySquare) ; draw polygon
...
(for ([i (in-range 127)])
  ...
  (draw-to-screen mySquare)) ; Draw the polygon
